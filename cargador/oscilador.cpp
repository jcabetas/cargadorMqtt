...
{
    // PA15TIM2CH1 es el pin de entrada al ondulador
...
void cargador::fijaAmperios(float amperios)
{
    ...
    // 30A == 50% => ancho=4000
    float Imin = iMinHR->getValor()/1000.0f;
    if (amperios<Imin)
        amperios = Imin;
    ancho = (uint16_t) (amperios*133.0f);
    pwmEnableChannel(&PWMD2, 0, ancho); // Onda para wallbox
    pwmEnableChannel(&PWMD2, 2, ancho-200); // detección tensión +12V
    pwmEnableChannel(&PWMD3, 3, ancho+3000); // detección tensión -12 V
}
...
//  Configure Timer 3 in trigger mode (SMS=110 in the TIM3_SMCR register).
    ...
    pwmEnableChannel(&PWMD3, 1, 6400); // TEST!!!  CH2 80% after start pulse (low side)
